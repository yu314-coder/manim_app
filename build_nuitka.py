...
def main():
    ...
    parser.add_argument("--build-type", type=int, choices=[1, 2, 3, 4, 5, 6, 7], help="Build type: 1=onefile, 2=standalone, 3=debug, 4=both silent, 5=minimal, 6=full latex, 7=enhanced latex")
    ...
    if args.build_type:
        ...
        if USE_ASCII_ONLY:
            ...
            print("7. Enhanced LaTeX build (NEW - improved LaTeX bundling)")
        else:
            print("1. 🔇 Silent onefile build (single .exe)")
            print("2. 📁 Silent standalone build (directory)")
            print("3. 🐛 Debug build (with console)")
    ...
    if choice == "1":
        ...
        print("\n" + ("Building with ENHANCED LaTeX support..." if USE_ASCII_ONLY else "🚀 Building with ENHANCED LaTeX support..."))
        exe_path = build_standalone_with_full_latex_v2(jobs=jobs, priority=process_priority)
        success = exe_path is not None
    else:
        if USE_ASCII_ONLY:
    ...
    if success:
        ...
        if choice == "5":
            ...
            if USE_ASCII_ONLY:
                print("ENHANCED LaTeX BUILD: Revolutionary LaTeX integration!")
                print("   NEW FEATURES:")
                print("   - Automatic TinyTeX download and bundling")
                print("   - Intelligent LaTeX environment setup")
                print("   - Robust error handling with fallbacks")
                print("   - Portable LaTeX distribution (~250MB)")
                print("   - Works without external LaTeX installation")
                print("   - Enhanced Manim LaTeX compatibility")
                print("   - Smart diagnostics and error recovery")
                print("   GUARANTEE: LaTeX will work out of the box!")
            else:
                print("🚀 ENHANCED LaTeX BUILD: Revolutionary LaTeX integration!")
                print("   🆕 NEW FEATURES:")
                print("   ✅ Automatic TinyTeX download and bundling")
